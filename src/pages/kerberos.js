...
const Kerberos = props => (
    <Layout>
        ...
        <p>Kerberos pretends your company network is filled with spies, and still provides you a way to securely log onto that LAN (authentication) without fear of anyone on the network being able to eavesdrop and steal your authenticate (password). Not only does Kerberos verify the client accessing accessing the file server is who they say they are, Kerberos authenticates to the client. It all goes on in the background, most people using Kerberos have no clue they are using it. All set up by the network admins, the heros we deserve. </p>
        ...
        Our hero Scotty Bob fires up his trusty desktop computer (Running Linux, obviously. Scotty is no plebe) and decides to log onto a local file server connected to the company LAN to see if the new hires actually got anything done, even though he already knows the answer in his heart (that's a big fat "nope"). Scotty Bob is greeted with an authentication screen when he tries to access the file server asking for his username and password, Scotty Bob types them, hits the enter key, and starts to take a sip of his Code Red Mountain Dew while Kerberos works magic in the background. <br></br>
        ...
        <h3>At long last, Scotty Bob receives his secure access token!</h3>
                ...
                <p>You might see Kerberos compared to other protocols. </p>
