...
const Chap = props => (
    <Layout>
        ...
        <p>These are the two different types of encryption algorithms. Symmetric is very simple, it uses the same key to encrypt that it uses to decrypt.<Img  fluid={props.data.front2.childImageSharp.fluid} />
            ...
            <p>Besides key exchanges, asymmetric encryption can be used for digital signature signing. This provides authentication that the message was from you, and integrity that the message content was not altered during transit. Some asymmetric algorithms can do both, while others focus on just one. <br></br>
            ...
            <p>The algorithm is basically just a program that uses your private key (also called your password sometimes) to produce a unique cipher which encrypts your data. There are a few of these in common use, and each will be gone over later in more depth. The ones that CompTIA wants you to know about are:  DE, RSA, DSA, ECC, PGP, and GPG. Some are used for the key exchanges, but some are also used only for signing digital signatures. You can skip ahead to section 6.2 and read about them: <Link to="/asymmetric"> Asymmetric Algorithms</Link></p>

            <h3>Symmetric encryption algorithms</h3>
            <p>
                The symmetric encryption algorithms are often what is used to encrypt and decrypt data sent between two parties, after they have used the key exchange to securely obtain the same symmetric encryption key. Remember, symmetric encryption uses the same key for both the encryption and decryption, so the key must be kept secure. That is data in transit, such as in an email. Symmetric encryption is also used to encrypted data at rest, your secret meme stash file on your laptop, for example. In this case you would encrypt it and keep the key to yourself, not handing it out to anyone. There are a bunch of symmetric encryption algorithms, the ones the CompTIA has deemed worthy of study are: AES, DES, 3DES, RC4, Blowfish, and Twofish. You will see these constantly. The are covered in section 6.2, coming up after you finish the current 6.1 section. You Can wait or skip ahead to read about them here:
                <Link to="/symmetric"> Symmetric Algorithms</Link></p>
...
<Link to="/cryptography">Previous: Cryptography</Link> <Link style={{ float: `right`}} to="/hashing">Next: Hashing</Link>
    </Layout>
)
export default Chap
